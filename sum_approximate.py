...
def initilization(n,m,d,R,p,bmax,s,alpha):
    ################ auxiliary variables
    ...
    for i in range (n):
        ...
        for j in range (m):
            dij[index][0] = s[i]*d[i] / R[i][j] + zedge
            Dij[index][0] = s[i]*d[i] / R[i][j] + zedge - alpha * gompertz_local(s[i],True)
...
def scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha):
    ...
    try: 
        ...
        slist = random_compression(n)
    print("A solution for s is", slist)
    return slist

def sdr_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations):     
    ...
    # constraints += [X>= 0]   # Convex Relaxation 0<=x_i,y_{ij}<=1
    ...
    # xscs = X.value
    # print("The SCS optimal value is", prob.value)
    # prob.solve(solver="MOSEK")
    # print("status:", prob.status)
    # xmosek = X.value 
    # print ("equal:," , np.array_equal(xscs,xmosek))
    # print("The Mosek optimal value is", prob.value)
    # print (np.diag(X.value)) 
...
def convex_offloading_new(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations,alpha):     
    X = cp.Variable((p))
    A0 = np.delete(B0, -1 , 0)
    A0 = np.delete(A0, -1, 1) 
    b0 = B0[-1]
    ...
    for i in range(n):
        column_to_be_added = np.zeros([p,1])
        column_to_be_added[i,0] = 1
        for k in range (m):
            column_to_be_added[i*m+n+k,0] = 1 
        A3.append(column_to_be_added)
    A4 = []
    for i in range (m):   # we have m constraints of this form
        column_to_be_added = np.zeros([p,1])
        for k in range (n):
            column_to_be_added[k*m+n+i,0] = 1
        A4.append(column_to_be_added)

    constraints= []
    constraints += [cp.quad_form(X,A0)+ c0.T@X<= Lmax*n]
    constraints += [d0.T@X <= -Amin*n]
    constraints += [A3[i].T @ X == 1 for i in range(n)]
    constraints += [A4[i].T @ X <= bmax[i] for i in range(m)]
    
    constraints += [X[i] >= 0 for i in range(p)]
    constraints += [X[i] <= 1 for i in range(p)]
    prob = cp.Problem(cp.Minimize(1/n*(cp.quad_form(X,A0)+b0.T@X)),
                    constraints)
    ...
    # print(np.any((X.value > 0) & (X.value < 1)))
    # print(np.any((X.value < 0)))
    # print(np.any((X.value > 1)))
    # violators = X.value[(X.value <= 0) | (X.value >= 1)]
    # print("The following values violate the condition:", violators)

    # print(X.value)
    Xstar = X.value
    ylist = []
    for i in range (n):
        xcandidate = X.value[i]
        ylist = X.value[n+i*m:n+i*m+m]

        if all(xcandidate >= item for item in ylist):
            xcandidate = 1
            ylist = [0 for item in ylist]
        else: 
            xcandidate = 0 
            ylist = [1 if item == max(ylist) else 0 for item in ylist]
        Xstar[i] = xcandidate
        Xstar[n+i*m:n+i*m+m] = ylist
    # print (Xstar)
    # print (bmax)
    # import time
    # time.sleep(30)
    ...
    total_cost, Lbest, Amax = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
    # print ("solution ", sdr_solution)

        # ksi = np.random.multivariate_normal(np.zeros(p), Xstar, tol=1e-6)
    minimum_obj = 10000000000000000
    pert = np.append(sdr_solution,1)
    pert = np.array([pert])
    Y = np.transpose(pert)*pert
    L = np.trace(B1 @ Y)
    A = np.trace(B2 @ Y)
...
def convex_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations):    
    ...
    constraints += [cp.trace(B3[i] @ X) == 1 for i in range(n)]
    constraints += [cp.trace(B4[i] @ X) <= bmax[i] for i in range(m)]
    constraints += [X[i] >= 0 for i in range(p+1)]
    # constraints += [X[i] <= 1 for i in range(p+1)]

    # constraints += [cp.trace(B5[i] @ X) >= 0 for i in range(p)]
    # constraints += [cp.trace(B5[i] @ X) <= 1 for i in range(p)]

    # constraints += [X[i] <= 1 for i in range(m)] 
    # constraints += [X[i] >= o for i in range(m)]  
    ...
    # print ("Rank of SDR solution is: ", rank)
    # return X.value
    # ksi = np.random.multivariate_normal(np.zeros(p), X.value[:-1,:-1], tol=1e-6)
    # sdr_solution = ksi
    # print (sdr_solution)
    # Lbest = np.trace(B1 @ X.value)
    # Amax = np.trace(B2 @ X.value)
    ############# mapping to feasible solution 
    iteration_best = -1
    Xstar = X.value
    Xstar = Xstar[:-1,:-1]
...
def random_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,Lmax,Amin):
    condition = True
    while (condition): 
        condition = False
        ...
        #         condition= True
        # total_cost, L, A = calculate_cost(n,m,solution,B0,B1,B2,B3,B4,Lmax,Amin)
        # if L >Lmax or A <= Amin:
        #     condition= True
        # print (solution)
    # print (check_bmax)
...
def random_compression(n): 
    slist = np.random.uniform(0.1,1,size=(n,))
    return slist

def offload_local(n,m):
    xlist =  [1]*n
    ylist = [] 
    for i, val in enumerate(xlist):
        sublist = [0] * m
...
def implement_brute_force(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin): 
    ############## Implement brute force solution....
    A = []
    lists = [[0, 1]] * (n*m+n)
    for sublist in itertools.product(*lists):
        mode = True
        xi = sublist[:n]
        yij = [sublist[n+i*m:n+(i+1)*m] for i in range(n)]
        for i in range(n):
            if (xi[i] + sum(yij[i])) != 1:
                # print("Constraint 1 violated for i = ", i)
                mode = False
                
        # check constraint 2
        for i in range(n):
            for j in range(m):
                if not (yij[i][j] == 0 or (yij[i][j] == 1 and xi[i] == 0)):
                    # print("Constraint 2 violated for i = ", i, " and j = ", j)
                    mode = False
        if mode == True:
            A.append(list(sublist))
    ...
    # print ("minimum objective ", minimum_obj)
    return solution[0][:-1], L, A

def calculate_cost(n,m,solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha):
...
def main(n,m,seed,mode,Lmax,Amin,iterations,r,alpha,b):
    p = n*m+n
    np.random.seed(seed)

    ###################### constants
    d = np.random.uniform(1,2,size=(n,))
    R = np.random.uniform(r[0],r[1],size=(n,m))
    bmax = np.random.randint(int(n/b[0]), int(n/b[1])+1,size=(m,))
    # bmax = np.random.uniform(n*n,size=(m,))
    ...
    while (True):
        ...
        B0,B1,B2,B3,B4,B5 = initilization(n,m,d,R,p,bmax,slist,alpha)
        if mode=="local_offloading" :
            sdr_solution = offload_local(n,m)
            total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            break ;
        elif mode=="remote_offloading_random_compression":
            sdr_solution = offload_remote(n,m)
            total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            break ;
        elif mode=="remote_offloading_decide_compression":
            sdr_solution = offload_remote(n,m)
            # Lmax = 1.9 # 0.3 #seconds
            # Amin = 0.1 #0.7 
            slist = scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha)
            B0,B1,B2,B3,B4,B5 = initilization(n,m,d,R,p,bmax,slist,alpha)
            total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            break ;            
        elif mode=="random_offloading_random_compression":
            sdr_solution = random_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,Lmax,Amin)
            total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            break ;   
        elif mode=="random_offloading_decide_compression":
            sdr_solution = random_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,Lmax,Amin)
            # Lmax = 1.9 # 0.3 #seconds
            # Amin = 0.1 #0.7 
            slist = scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha)
            ...
            total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            break ; 
        elif mode=="sdr_offloading_random_compression":
            # Lmax = 1.9 # 0.3 #seconds
            # Amin = 0.1 #0.7 
            sdr_solution, Lbest, Amax = sdr_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations) 
            slist = random_compression(n)
            B0,B1,B2,B3,B4,B5 = initilization(n,m,d,R,p,bmax,slist,alpha)
            ...
            Acurrent = Amax
            counter +=1
        elif mode=="sdr_offloading_decide_compression":
            # Lmax = 1.9 # 0.3 #seconds
            # Amin = 0.1 #0.7 
            sdr_solution, Lbest, Amax = sdr_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations) 
            slist = scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha)
            ...
            Acurrent = Amax
            counter +=1
        elif mode=="convex_offloading_decide_compression":
            # Lmax = 1.6 # 0.3 #seconds
            # Amin = 0.1 #0.7 
            # sdr_solution, Lbest, Amax = convex_offloading(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,True) 
            sdr_solution, Lbest, Amax = convex_offloading_new(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin,iterations,alpha) 
            slist = scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha)
            B0,B1,B2,B3,B4,B5 = initilization(n,m,d,R,p,bmax,slist,alpha)
            if (Lbest-alpha*Amax <= Lcurrent-alpha*Acurrent + epsilon) and (Lbest-alpha*Amax >= Lcurrent-alpha*Acurrent - epsilon):
                total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
            ...
            sdr_solution, Lbest, Amax = implement_brute_force(n,m,d,R,p,bmax,B0,B1,B2,B3,B4,B5,Lmax,Amin) 
            slist = scaling_problem(n,m,d,R,p,bmax,sdr_solution,Lmax,Amin,alpha)
            B0,B1,B2,B3,B4,B5 = initilization(n,m,d,R,p,bmax,slist,alpha)
            if (Lbest-alpha*Amax <= Lcurrent-alpha*Acurrent + epsilon) and (Lbest-alpha*Amax >= Lcurrent-alpha*Acurrent - epsilon):
                total_cost, Lfinal,Afinal = calculate_cost(n,m,sdr_solution,B0,B1,B2,B3,B4,Lmax,Amin,alpha)
    ...
    x_variables = sdr_solution[0:n]
    #calculate local average for Latency and accuracy
    local_executed_requests_count = x_variables.tolist().count(0)
    offloaded_ratio = local_executed_requests_count/len(x_variables)
    print ("offloaded ratio:", offloaded_ratio)
    print ("number of offloaded requests:", local_executed_requests_count)
    print ("number of  requests:", n)
    print ("offloaded div sum bmax", local_executed_requests_count/int(sum(bmax)))
    print ("sum bmax", int(sum(bmax)))
    print ("Lfinal", Lfinal)
    print ("Afinal", Afinal)

    print ("final solution", sdr_solution)
    y_solution = sdr_solution[n:]
    communication = []
    decision_of_offloading = [[] for i in range(m)]
    for i in range (n):
        for j in range(m):
            if y_solution[i*m+j] == 1 :
                decision_of_offloading[j].append(i)
            communication.append((y_solution[i*m+j]*d[i]*slist[i])/R[i][j])
    # print ("data size", d)
    # print ("data rates", R)
    print ("compression", slist)
    print ("communication_time:", communication)
    try: 
        average_communication = sum(filter(None, communication)) / len(list(filter(None, communication)))
        average_computing = (Lfinal*n - sum(communication) )/n
    except ZeroDivisionError:
        average_communication = 0 
        average_computing = Lfinal
    print ("Average Communication:", average_communication)
    print ("decision of offloading for each server which reqeusts:",  decision_of_offloading)
    return total_cost, Lfinal,Afinal, average_communication, Lfinal - average_communication

if __name__ == "__main__":
    # main(2,2,3,"lower_bound",1.9,0.1)
    main(5,5,3,"random_offloading_decide_compression",1.9,0.1)
